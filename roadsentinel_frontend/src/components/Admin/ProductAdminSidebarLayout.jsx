...
export default function ProductAdminSidebarLayout({ children, activeSection, onSectionChange }) {
  const { logoutAdmin } = useAdminAuth();
  const navRef = useRef(null);

  useEffect(() => {
    // Smooth entry for navigation bar layout
    gsap.fromTo(
      navRef.current,
      { opacity: 0, y: -20 },
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-screen overflow-hidden bg-[#767676] font-sans text-gray-800">
      ...
      >
        ...
        <div className="flex md:flex-col items-center justify-between md:items-stretch p-4 md:p-6 border-b border-gray-100">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-lg md:text-xl font-bold tracking-tight">ADMIN</span>
          </div>
        ...
        {/* Dynamic Horizontal Scroll (Mobile) / Vertical Stack (Desktop) Nav Items */}
        ...
        {/* Desktop Base Footer Logout Option */}
        <div className="hidden md:block p-4 border-t border-gray-100">
          <button 
            onClick={logoutAdmin} 
            className="w-full flex items-center space-x-2 px-4 py-2 text-sm font-medium text-gray-500 hover:text-red-600 transition-colors"
          >
      ...
      {/* RENDER BODY FRAGMENT CONTAINER */}
